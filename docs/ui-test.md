...
**跑红也要问一句「红在哪一条」。** 上面三种都是「跑绿」，还有第四种是**崩溃红冒充断言红**：
把 `product.sku` 改成读规格级字段做变异，三处初始化点全红——看着像验过了，实际那三行的
作用域里根本没有 `sku` 变量（它是 `const`，声明在后面各分支的块里），红的是
`ReferenceError: sku is not defined`，以及两条毫不相干的「库存不足」。**没有一条在说货号。**

判据：**报错文案是不是在说你改的那件事**。换一种不依赖作用域、不会崩的变异形态
（例如把值换成 `'__MUT__'` 这种毒值字面量），红就只可能红在断言上。

顺带一条同样容易骗人的：判「红在哪一条」时**别用太窄的 grep**。同一批里我用
`/(销售|进货)…的货号要取商品级/` 去筛，七处被判成「红在别处」，实际都精确红在货号断言上，
只是文案里「初始值」「applyAdjust」这些词没被正则捕获——**取证工具本身也会制造假结论**。

### 为什么有的路径「天然测不出」

那批里 `allocateBlankLine` 藏了三轮，最后发现不是大家忘了写断言：**既有待加工夹具的
`product.sku` 几乎都是空串**，而空串时 `x || product.sku` 和 `product.sku` 算出来完全一样，
变异改了也看不出差别。**是夹具的形状让这条路测不出来。** 写断言前先看一眼夹具：
被测差异在这份数据上表现得出来吗？
