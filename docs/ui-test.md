...
**还有第三种跑绿：变异打中了、断言也是活的，但它一直活在别的路径上。**

2026-09-03 实测。`tests/slip-image.test.js` 用 7 个夹具逐条覆盖矩阵化的 7 条否决条件，
...
原因：那 5 条夹具都只有 2 行，而条件 6（有压缩收益 `2 + 行轴取值数 < 节行数`）要求至少 4 行。
...
**跑红也要问一句「红在哪一条」。** 上面四种都是「跑绿」，还有第五种是**崩溃红冒充断言红**：
