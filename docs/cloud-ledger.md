...
3. 给 `ledger_records` 建上面「集合」那条里列的 **6 条索引**（复合索引，字段顺序和升降序都不能改；漏一条就会退化成全表扫，条数一多就超时）。**这一步不是可选的**：#1–#5 是流水的每一次查询都要用的，当天漏任何一条都当场报错；#6 当前没有查询用，但它是 2b-3 清理的前置，趁这次一起预建，免得到时候再回来动生产索引。
