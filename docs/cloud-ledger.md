...
- 分页协议（`listRecords`）：
  ...
  - **不回 `total`**：流水页的类型 chip 和客户页都不显示条数。（2026-08-31 起「全部」那枚 chip 也不再挂 `totals.count` —— 加了时间段筛选之后，全店累计条数挂在一个只显示本月的列表上面是错的；条数改由摘要条那一档各自的窗口负责，而摘要条按设计只印金额不印笔数。）
...
**不需要第二个冻结开关。** 写路径从新云函数部署那一刻起就被 `assertRecordsReady` 挡住了（2b-3 起 `recordsPending` = 「既没有 `recordsMigratedAt`、也没有 `recordsSchema`」，见上面「集合」那一节；2b-3 之前是 `records.length > 0 && !recordsMigratedAt`），不要再加一个 `ledgers.migration.state`——两个冻结口径迟早会打架。
...
- **`mode:'rollback'`**：已 `done` 之后发现不对，只清 `recordsMigratedAt` 和 `migration`，老数组还在，**读**立刻退回老路径；
  ...
  **写是冻着的**——`recordsPending` 重新为真，`assertRecordsReady` 照旧拦下每一条写（实测回滚后记账和删单都报「本店账本还没完成流水升级，暂时不能记账」），回滚是回到停摆态、不是重新开张。**这是显式动作，不要让人去控制台手改生产文档。**代码**不检查 `migration.phase`**：`writing` / `verifying` 中途也能调，效果同样是回到停摆态（`cursor` / `verifyCursor` 跟着 `migration` 一起清掉，等于放弃这次尝试）；前置条件只有一条真实的——老数组非空，为空的店（跑过 `dropLegacy`、点过「清空数据」、点过「恢复清空前数据」）当场报「没有可回滚的老流水」。回滚后**重跑必须带 `restart: true`**（集合里的文档还在，不带 `restart` 会被残骸检查拒绝），若回滚前已按新路径记过账，`restart` 会撞 V7/V2（见上面「失败恢复」），只剩 `newBook: true` 一条路。
  ...
  - 老数组为空时报的**四种**来路：跑过 `dropLegacy`、店主点过「清空数据」（`clearAll`）、店主点过「恢复清空前数据」（`restoreCleared`）、**跑过 2b-3 之后记过账**（记账不再携带老数组，第一笔就把它删了——2b-3 之后这是最常见的那一种）。中间两种在 `ledger_clears` 的快照文档里还留着老数组的副本。
- **`mode:'dropLegacy'`**：一个事务把 `ledgers.records` 置空。**跑完就没有 O(1) 回滚了**。迁移**不会**让账本文档变小，5 MB 墙仍在。

  > **2b-3 起它从「可选优化」变成「上线前置」，而且顺序不能反：先在每家店跑完 `dropLegacy`，再部署 2b-3 版本的云函数。** 顺序反了的后果：2b-3 的 `applyMutation` 不再携带 `records`、`putLedger` 是整文档 `set()`，所以**每家店的第一笔记账就是一次没有任何守卫的隐式清空**——下面那两道闸（`recordsMigratedAt` 非空、集合数得着且非空）一道都不会跑，也不会报错，那一笔账落下去的同一瞬间老数组就没了。**跑晚了则是白跑**（字段已经被记账删掉，回包 `dropped` 恒为 0）。2b-3 之前的定位是「默认不跑，只在账本文档逼近 5 MB（P12）时当晚跑」，那句已经不成立了。
...
- **`ledgers.records` 只活到下一笔账为止（2b-3）。** 迁完那一刻 `migrateRecords` 仍然故意不删它，但 `applyMutation` **不再**把它带过每一次记账，而 `putLedger` 是整文档 `set()`——所以迁完之后**任何一次记账**（不只是清空 / 恢复）都会把它从文档里删掉。2b-3 之前这一条是反的（「故意留着当 O(1) 回滚路」），`tests/ledger-records.test.js` 第 9 节那条断言已经跟着反过来钉。
  - **它不再是任何一条退路的依仗。** 「整体不对就部署存档的旧函数包」这条深层退路，在 2b-3 部署 + 任意一笔记账之后，对**所有店**都不成立——旧包读的是 `ledgers.records`，它在这些店上会读出一本空账。（2b-3 之前只有三类店回不去：跑过 `dropLegacy` 的、用过「恢复清空前数据」的、点过「清空数据」的。）数据本身没丢（还在 `ledger_records` 里；`clearAll` 存进 `ledger_clears` 的快照文档里还带着老数组的副本），但旧包**看不见集合里那份、也看不见快照里那份**。这条退路是用户明确拍板放弃的，不要再按「还留着」来排期。
  - 顺带作废的还有一条老写法：「三个动作会把它清空」（`dropLegacy` / `restoreCleared` / `clearAll` → `switchBook`）。`switchBook` 和 `restoreCleared` 里那两行 `next.records = []` 2b-3 已经删掉——`next` 来自 `listsOf()`，它根本不产出 `records` 键，留着那行的作用会从「清空」翻转成「凭空塞回一个空数组」。
...
**删前校验用 `countAll === aggregate.count`，两类快照通用**：A 类（老快照被转换过来的）`aggregate = foldTotalTerms(merged)`，所以 `aggregate.count` 就是转换那一刻 `countAll` 校验过的归并条数；B 类（迁移之后、`dropLegacy` 之前点「清空数据」存的）`aggregate` 是被封存账套那一刻的增量维护值，而被封存的账套此后不再变化。**B 类 2b-3 起不再新增，只剩存量遗留**：2b-3 的记账不再携带 `ledgers.records`（迁移后第一笔账就把这个 key 抹掉），空数组既走不过 `apply.snapshotLists` 的 `if (legacy.length)`，也走不过 `core.clearDoc` 的 `if (snapshot.records && snapshot.records.length)`——**云路径的快照文档写不写 `records` 键，最终由 `clearDoc` 那道决定**（端到端用例 D10 咬住的是它；`snapshotLists` 那道在内存模式下没有 `clearDoc` 兜底、是唯一的一道，由 D10b 直接调函数单独锁）——于是 `clearAll` 生成的新快照**没有 `records` 这个字段**（缺字段，不是空数组），一律走「本来就没有数组」那一支报幂等 `skipped`——所以跑完满屏 `skipped` 是常态，`failed > 0` 才是真出事。**判据闸仍然为存量而在，不许改**：生产上还有 3 份带数组的快照待清（卓祥 1 份、`msxeubh4c6d5f9` 2 份），判据一松，它们一份都清不掉。**不许改成「重新归并 `records` 数组再比条数」**：B 类的数组是迁移前的过期子集（封存前记的新账在集合里、不在数组里），归并出来的条数本来就不等于集合里的条数，那么判会把 B 类全判成失败。判据是严格相等——少一条不行，多一条也不行（多出来的恰恰说明这本账套已不是快照冻结时的那本）。
...
→ **【真云实测②：单事务写入条数上限 + 事务超时】**① **写入条数这一半已经测过一次（2026-08-24，演示店 `mt33kfi77idxpw`）**：改一张挂着 90 张退货单的销售单的单价（事务写 `ledgers` 1 + 目标 1 + 退货单 90 = 92 条）**确定性失败**，服务端报 `[ResourceUnavailable.TransactionNotExist]`「transaction must be commit or abort in 30 seconds」，而函数耗时只有 12.3 / 11.5 秒——**所以真实边界不是那 30 秒。后来在同一家店上做了二分**：**22 条写入通过（9.7 秒）、47 条失败（11.3 秒）、92 条失败（12–16 秒）**。11.3 秒失败而 9.7 秒通过，**时间至此被彻底排除**，是条数或体积。**2026-08-25 做了对照实验，两者已经分开：是体积，不是条数。** 同样的 47 条单事务写入（1 `ledgers` + 1 销售单 + 45 退货单），在一家新建空店（`ledgers` **5,932 字节**）上 **通过，3.6 秒**；在演示店（`ledgers` **3.6 MB**）上 **失败，11.3 秒**。写入条数完全相同、唯一变量是账本大小，结果一成一败。
>
> 旁证：造这 45 张退货单时，小账本店每张 **897 毫秒**、演示店 **8.8 秒**——两边写入条数一模一样，差近十倍。事务耗时主要就是重写整个 `ledgers` 撑起来的。
>
> **所以给 `SALE_RETURNS_MAX` 定任何常数都不安全**：阈值随账本大小浮动，同一个 N 在小店过、在大店不过，而账本会随着做生意一直长。正确的修法是**把整体重算搬出事务**（或者至少别在同一个事务里重写整本账），而不是把 200 换成一个更小的数。**尚未做的**：在不同时段各测一次排除并发 / 负载因素，理由和口径见 `ledger-records.js` 的 `SALE_RETURNS_MAX` 与 `index.js` 事务改写处那两段注释。② **事务超时这一半还没测**：对着测试店的全量流水跑一次 `recomputeAggregates`——它在**一个事务里**做最多 `RECOMPUTE_MAX_RECORDS` 5000 条 / 50 次串行分页查询，看会不会撞超时（`config.json` 的 `timeout` 现在是 60 秒，不是当初写这条时的 20 秒）。
...
**回滚**：某店 `failed` → 不动，该店停摆，无错账；某店 `done` 后发现不对 → `mode:'rollback'`（O(1)，该店**仍停摆**——只有读退回老路径，写仍被 `assertRecordsReady` 冻着；集合里的文档留着，**重跑必须带 `restart: true`**，回滚前记过账就连 `restart` 也会撞 V7/V2、只能 `newBook: true`；**迁完之后已经记过账就会被拒绝**，那些账只在集合里，回滚看不见它们——真要回滚得先另行备份再带 `force: true`；反过来，迁完之后**只删过单**的店回滚会**放行**，被删的单在老路径上复活，守卫不管这一侧。**2b-3 部署之后这条路更窄**：记过账的店老数组已经被首笔账删掉，`rollback` 当场报「没有可回滚的老流水」，`force` 也绕不过——只有一笔账都没记过的店还回得去）；整体不对 → 部署阶段 0 存档的旧函数包（已跑过 `dropLegacy` / 清空 / 恢复的店回不去；**2b-3 部署 + 任意一笔记账之后，所有店都回不去**）；聚合漂了 → `recomputeAggregates`。
