...
| 不进代码包 | `.git/`、`tests/`、`node_modules`、`docs/`、`cloudfunctions/`、`scripts/`、仓库说明与 npm 清单已在 `packOptions.ignore` |
...
改完配置后，至少走一遍：看板、商品（含预警筛选）、进货、销售、客户、流水。确认没有依赖「所有页面 JS 启动时都会执行」这种隐式行为。客户是 tab 页，必须留在主包。
