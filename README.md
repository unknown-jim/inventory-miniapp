...
4. 在云开发控制台创建集合 `shops`、`members`、`ledgers`、`ledger_records`、`ledger_clears`。`ledger_records` 的 6 条索引用 `node scripts/wxcloud-ensure-indexes.js` 建；五张表权限用 `node scripts/wxcloud-ensure-acl.js` 设成仅管理端可读写（部署脚本末尾都会跑）
