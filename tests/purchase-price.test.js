...
//
// 不用「总块数」那个常量：常量会漂（加了块忘了改，闸就静默放行），而用「数一遍
// async function」来自检又会把判据自己的那份也数进去。进出配对不需要知道总数，
// 加块时照抄这一对即可。
//
// 它抓得住：块内提前 return、await 的 promise 永不 settle、异常被吞而 `ended` 在 try 内。
// 它**抓不住任何让进出仍然相等的形态**——复审实测三种：
