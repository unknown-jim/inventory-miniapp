...
- 过程文档（调查、审计、派工 prompt、交接回报、分期计划）不进本仓库，放 `~/work/inventory-miniapp-handoffs/`。命名与长寿计划文档的写入规则见 [docs/git-workflow.md](docs/git-workflow.md#过程文档不进仓库)。
...
- 操作界面不要写小于 `--fs-caption`（24rpx）的字号、不要写小于 `--tap-min`（88rpx）的可点高度。新页面用 `app.wxss` 共用布局类，根节点 `class="page"`。送货单的**单据版式**除外（弹层里给店主点的操作控件不算单据版式，照常套这套变量）。详见 [docs/ui-scale.md](docs/ui-scale.md)。
